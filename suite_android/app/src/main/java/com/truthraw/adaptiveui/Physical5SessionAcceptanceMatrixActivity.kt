...
class Physical5SessionAcceptanceMatrixActivity : Activity() {
    ...
    private fun buildUi(): View {
        ...
        body.addView(label("TruthRaw v0.68 · Physical-5 session matrix", 21f, true))
        body.addView(label(
        ...
        body.addView(space(10))
        status = label("Nog geen v0.68 report.", 10f, false)
        body.addView(status)
    ...
    private fun runMatrix() {
        if (checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            status.text = "v0.68 vraagt CAMERA-toestemming voor session-acceptance tests."
    ...
    private fun executeMatrix() {
        ...
        status.text = "v0.68 start complete matrix…"
        Thread {
            val report = runCatching { buildReport() }.getOrElse { e ->
                JSONObject()
                    ...
                    .put("classification", "V068_FATAL_ERROR")
    ...
    private fun buildReport(): JSONObject {
        ...
        for ((index, spec) in CANDIDATES.withIndex()) {
            runOnUiThread {
                status.text = "v0.68 test \${index + 1}/\${CANDIDATES.size}: \${spec.shortName}"
            }
        ...
        return JSONObject()
            ...
            .put("classification", "PHYSICAL5_HAL_SESSION_ACCEPTANCE_MATRIX__NO_CAPTURE")
    ...
    private fun refreshStatus() {
        ...
        if (!file.exists()) {
            status.text = "Nog geen v0.68 report."
            return
        }
        val report = runCatching { JSONObject(file.readText()) }.getOrNull()
        if (report == null) {
            status.text = "v0.68 report bestaat maar kon niet worden gelezen."
            return
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        runCatching {
            ...
            status.text = "v0.68 JSON opgeslagen."
        }.onFailure {
    ...
    companion object {
        private const val SCHEMA = "truthraw.physical5-session-acceptance-matrix.v0.68"
        private const val AUTHORITY = "CAMERA2_HAL_SESSION_ACCEPTANCE_MATRIX_NO_CAPTURE"
        private const val REPORT_FILENAME = "TRUTHRAW_PHYSICAL5_SESSION_ACCEPTANCE_MATRIX_v068.json"
        private const val REQUEST_CAMERA_PERMISSION = 66862
        private const val REQUEST_SAVE_JSON = 66863
