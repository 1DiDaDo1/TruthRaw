...
/**
 * v0.66 narrow physical-5 HAL/session acceptance probe.
 *
 ...
 * access occurs.
 */
class Physical5MasterFilmSessionAcceptanceActivity : Activity() {
    ...
    private fun buildUi(): View {
        ...
        body.addView(label("TruthRaw v0.66 · MasterFilm session acceptance", 21f, true))
        body.addView(label(
            "Nauwe HAL/session-test op physical camera 5. Eerst een identieke control-session zonder vendorwaarde; " +
                "daarna alleen MasterFilmSensorType=intArrayOf(1). Geen capture of pixeltoegang.",
        ...
        body.addView(space(10))
        status = label("Nog geen v0.66 report.", 10f, false)
        body.addView(status)
    ...
    private fun runProbe() {
        if (checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            status.text = "v0.66 vraagt CAMERA-toestemming voor de session-acceptance test."
    ...
    private fun executeProbe() {
        status.text = "v0.66 configureert eerst control-session…"
        ...
        Thread {
            val report = runCatching { buildReport() }.getOrElse { e ->
                JSONObject()
                    ...
                    .put("classification", "V066_FATAL_ERROR")
    ...
    private fun buildReport(): JSONObject {
        ...
        if (device == null) {
            return out
                ...
                .put("classification", "V066_CAMERA_OPEN_FAILED")
        ...
        try {
            ...
            if (!control.configured) {
                return out
                    ...
                    .put("classification", "V066_CONTROL_SESSION_FAILED__CANDIDATE_NOT_ATTEMPTED")
                    .put("boundary", BOUNDARY)
            }
            ...
            val candidate = configureSession(
                ...
                label = "MASTER_FILM_SENSOR_TYPE_INT_ARRAY_1"
            )
            ...
            return out
                .put("classification",
                    if (candidate.configured)
                        ...
                        "MASTERFILMSENSORTYPE_INT_ARRAY_1__HAL_SESSION_REJECTED_OR_FAILED__NO_CAPTURE")
                .put("boundary", BOUNDARY)
        } finally {
            device.close()
        }
    ...
    private fun refreshStatus() {
        ...
        if (!file.exists()) {
            status.text = "Nog geen v0.66 report."
            return
        }

        val report = runCatching { JSONObject(file.readText()) }.getOrNull()
        if (report == null) {
            status.text = "v0.66 report bestaat maar kon niet worden gelezen."
            return
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        runCatching {
            ...
            status.text = "v0.66 JSON opgeslagen."
        }.onFailure {
    ...
    companion object {
        private const val SCHEMA = "truthraw.physical5-masterfilm-session-acceptance.v0.66"
        private const val AUTHORITY = "CAMERA2_HAL_SESSION_ACCEPTANCE_NO_CAPTURE"
        private const val REPORT_FILENAME = "TRUTHRAW_PHYSICAL5_MASTERFILM_SESSION_ACCEPTANCE_v066.json"
        private const val REQUEST_CAMERA_PERMISSION = 66662
        private const val REQUEST_SAVE_JSON = 66663
        private const val LOGICAL_ID = "0"
        private const val PHYSICAL_ID = "5"
        private const val TARGET_KEY = "com.hihonor.capture.metadata.MasterFilmSensorType"
