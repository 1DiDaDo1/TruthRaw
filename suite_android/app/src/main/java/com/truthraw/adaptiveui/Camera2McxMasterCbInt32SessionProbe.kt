...
/**
 * v0.35 single-variable session intervention for
 ...
 */
object Camera2McxMasterCbInt32SessionProbe {
    const val KEY_NAME = "org.codeaurora.qcamera3.sessionParameters.EnableMCXMasterCb"
    const val NATIVE_TAG_HEX = "0x801F0005"
    ...
    fun applyExperiment(
        ...
        val out = JSONObject()
            .put("schema", "truthraw.camera2-mcx-master-cb-int32-session-intervention.v0.35")
            .put("experiment", "ENABLE_MCX_MASTER_CB_SESSION_PARAMETER_INT32_ONE")
            ...
            .put("predecessorExperiments", "v0.33 and v0.34 both no measured RAW topology differential")
            .put("applied", false)

        if (!logicalSessionAdvertised) {
            out.put("classification", "BLOCKED_MCX_MASTER_CB_NOT_ADVERTISED_AS_LOGICAL_SESSION_KEY")
        ...
        val builder = runCatching { device.createCaptureRequest(CameraDevice.TEMPLATE_STILL_CAPTURE) }
            .getOrElse { e ->
                out.put("classification", "BLOCKED_MCX_MASTER_CB_INT32_BUILDER_CREATE_FAILED")
        ...
        if (setError != null) {
            out.put("classification", "BLOCKED_MCX_MASTER_CB_INT32_SET_FAILED")
        ...
        if (!builderReadbackPass) {
            out.put("classification", "BLOCKED_MCX_MASTER_CB_INT32_BUILDER_READBACK_MISMATCH")
            return ApplyResult(false, out)
        }

        val request = runCatching { builder.build() }.getOrElse { e ->
            out.put("classification", "BLOCKED_MCX_MASTER_CB_INT32_REQUEST_BUILD_FAILED")
                .put("error", "${e.javaClass.simpleName}: ${e.message}")
            return ApplyResult(false, out)
        }
        ...
        if (!requestReadbackPass) {
            out.put("classification", "BLOCKED_MCX_MASTER_CB_INT32_REQUEST_READBACK_MISMATCH")
            return ApplyResult(false, out)
        }

        val attachError = runCatching { config.setSessionParameters(request) }.exceptionOrNull()
        if (attachError != null) {
            out.put("classification", "BLOCKED_MCX_MASTER_CB_INT32_SET_SESSION_PARAMETERS_FAILED")
        ...
        out.put("sessionParametersAttached", true)
            ...
            .put(
                "classification",
                "MCX_MASTER_CB_INT32_ONE_ATTACHED_AS_SINGLE_VENDOR_SESSION_VARIABLE__SEMANTICS_UNPROVEN",
            )
