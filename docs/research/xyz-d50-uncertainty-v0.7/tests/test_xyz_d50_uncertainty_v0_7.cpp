...
#define REQUIRE(expr) do { \
    if (!(expr)) { \
        std::cerr << "REQUIRE failed: " #expr << " at line " << __LINE__ << "\n"; \
        return 1; \
    } \
} while (false)
...
int main() {
    // Matrix contract is explicit and fail-closed.
    {
        ...
        REQUIRE(!truthraw_v07::propagate_camera_rgb_uncertainty_to_xyz_d50_v0_7(
            full_input(), bad, out));

        auto bad2 = matrix({1,0,0,0,1,0,0,0,1});
        bad2.m[2] = std::numeric_limits<double>::quiet_NaN();
        REQUIRE(!truthraw_v07::validate_camera_to_xyz_d50_matrix_v0_7(bad2));
    }
    ...
    // Identity transform preserves a fully certified covariance exactly.
    {
        ...
        auto s = truthraw_v07::propagate_camera_rgb_uncertainty_to_xyz_d50_v0_7(
            full_input(), m, out);
        REQUIRE(s);
        REQUIRE(out.knowledge == XyzPropagationKnowledgeV07::ExactFullCovariance);
        REQUIRE(out.varianceKnownMask == 0x7u);
        REQUIRE(out.covarianceKnownMask == 0x7u);
        REQUIRE(out.fullPsdCertified);
        REQUIRE(near(out.variance[0], 4.0));
        REQUIRE(near(out.variance[1], 9.0));
        REQUIRE(near(out.variance[2], 16.0));
        REQUIRE(near(out.covariance[0], 1.0));
        REQUIRE(near(out.covariance[1], -1.0));
        REQUIRE(near(out.covariance[2], 2.0));
    }
    ...
    // Nontrivial transform: check independent hand-derived entries of M Sigma M^T.
    {
        ...
        auto s = truthraw_v07::propagate_camera_rgb_uncertainty_to_xyz_d50_v0_7(
            full_input(), m, out);
        REQUIRE(s && out.fullPsdCertified);
        REQUIRE(near(out.variance[0], 4.0));
        REQUIRE(near(out.variance[1], 36.0));
        REQUIRE(near(out.variance[2], 12.0));
        REQUIRE(near(out.covariance[0], 2.0));   // Cov(R,2G)
        REQUIRE(near(out.covariance[1], 2.5));   // Cov(R,R-G+0.5B)
        REQUIRE(near(out.covariance[2], -14.0)); // Cov(2G,R-G+0.5B)
    }
    ...
    // give variance 13; the valid outer interval is [1,25].
    {
        ...
        REQUIRE(s);
        REQUIRE(out.knowledge == XyzPropagationKnowledgeV07::ComponentVarianceBounds);
        REQUIRE(out.covarianceKnownMask == 0u);
        REQUIRE(std::isnan(out.covariance[0]));
        REQUIRE(out.varianceBounds[0].valid && !out.varianceBounds[0].exact);
        REQUIRE(near(out.varianceBounds[0].lower, 1.0));
        REQUIRE(near(out.varianceBounds[0].upper, 25.0));
        REQUIRE(std::isnan(out.variance[0]));
    }
    ...
    // unrelated RGB pair terms remain unresolved.
    {
        ...
        auto s = truthraw_v07::propagate_camera_rgb_uncertainty_to_xyz_d50_v0_7(p, m, out);
        REQUIRE(s);
        REQUIRE((out.varianceKnownMask & 0x1u) != 0u);
        REQUIRE(out.varianceBounds[0].exact);
        REQUIRE(near(out.variance[0], 11.0));
        REQUIRE(out.covarianceKnownMask == 0u);
        REQUIRE(!out.fullPsdCertified);
    }
    ...
    // Quantile-only / no-variance state remains unresolved; no sigma is invented.
    {
        ...
        auto s = truthraw_v07::propagate_camera_rgb_uncertainty_to_xyz_d50_v0_7(p, m, out);
        REQUIRE(s);
        REQUIRE(out.knowledge == XyzPropagationKnowledgeV07::Unresolved);
        REQUIRE(out.varianceKnownMask == 0u);
        REQUIRE(out.covarianceKnownMask == 0u);
        for (const auto& b : out.varianceBounds) REQUIRE(!b.valid);
    }
    ...
    // A non-PSD fully numeric v0.6 input must fail before propagation.
    {
        ...
        auto m = matrix({1,0,0,0,1,0,0,0,1});
        PixelXyzD50UncertaintyV07 out;
        REQUIRE(!truthraw_v07::propagate_camera_rgb_uncertainty_to_xyz_d50_v0_7(p, m, out));
    }
    ...
    // Dense tile preserves geometry and per-pixel knowledge state.
    {
        ...
        REQUIRE(s);
        REQUIRE(out.originX == 3 && out.originY == 7 && out.width == 2 && out.height == 1);
        REQUIRE(out.pixels.size() == 2);
        REQUIRE(out.pixels[0].knowledge == XyzPropagationKnowledgeV07::ExactFullCovariance);
        REQUIRE(out.pixels[1].knowledge == XyzPropagationKnowledgeV07::Unresolved);
    }
