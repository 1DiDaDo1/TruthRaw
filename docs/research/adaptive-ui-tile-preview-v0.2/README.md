...
TileNativeDngSource v0.1 requires both `sourceEvidenceId` and an explicit color binding before it opens. v0.2 does **not** yet have the final source-evidence hash/backplane identity or a calibrated arbitrary-camera color binding at UI selection time.

Therefore the JNI preview call uses two local sentinel strings:
...
This sentinel **must never be reused for scientific reconstruction**. A future scientific handoff remains fail-closed until the true source-evidence identity and accepted color binding are resolved and bound through the runtime/backplane path.
...
The JNI packet returns lightweight audit fields to the UI:
