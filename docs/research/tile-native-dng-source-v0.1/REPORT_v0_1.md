...
Promotion is permitted only from a new single-commit research branch created directly from unchanged `main`, after Tile-Native DNG Source v0.1 Integrity and Documentation Governance both pass on those exact final bytes. The clean-branch CI result is external repository authority; this report does not predeclare it.
