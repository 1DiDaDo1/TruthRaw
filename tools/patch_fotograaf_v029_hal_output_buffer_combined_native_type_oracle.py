...
# Reconstruct exact v0.20 source/payload authority. v0.29 is Stage-1-only diagnostics.
# It reuses the proven disposable NDK metadata validator for exactly one different
# advertised vendor key: HALOutputBufferCombined. No session parameters or capture are submitted.
ns = {}
exec(
    compile(
...
s = s.replace(
    ...
    'TruthRaw · Camera-5 v0.29 · HALOutputBufferCombined native type oracle',
)
s = s.replace('truthraw-200mp-v020', 'truthraw-200mp-v029')
s = s.replace('truthraw-v020-capability', 'truthraw-v029-capability')
s = s.replace(
    ...
    'Stage-1-only diagnostic: resolve HALOutputBufferCombined native metadata type with disposable NDK request metadata; no session parameters, capture session or capture submit',
)
s = s.replace(
    'saveJsonButton = button("200MP evidence JSON opslaan")',
    'saveJsonButton = button("v0.29 HALOutputBufferCombined native type-oracle JSON opslaan")',
)
