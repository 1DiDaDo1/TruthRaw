...
# Corrected v0.30 patch. The first v0.30 patch is intentionally preserved as failed-build
# provenance; its only failure was an overly specific UI label anchor. This version reconstructs
# the exact v0.20 authority and uses stable structural anchors instead.
ns = {}
exec(
    compile(
...
# Capture label changed upstream after v0.11; change it opportunistically without making it an anchor.
s = s.replace('Stap 3 · CAPTURE + SEAL 200MP RAW · 16320×12288', 'Stap 3 · MATRIX CAPTURE · PHYSICAL 5 · 16320×12288')
...
if button_field_needle not in s:
    raise SystemExit('v0.30b matrix button field anchor not found')
...
if ui_add_needle not in s:
    raise SystemExit('v0.30b matrix UI addView anchor not found')
...
if method_anchor not in s:
    raise SystemExit('v0.30b helper-method anchor not found')
...
if capture_start_needle not in s:
    raise SystemExit('v0.30b capture-start anchor not found')
...
if support_needle not in s:
    raise SystemExit('v0.30b matrix insertion anchor not found')
...
if report_needle not in s:
    raise SystemExit('v0.30b evidence insertion anchor not found')
...
if status_needle not in s:
    raise SystemExit('v0.30b final status anchor not found')
...
if success_needle not in s:
    raise SystemExit('v0.30b success advance anchor not found')
...
if fail_needle not in s:
    raise SystemExit('v0.30b failure-path anchor not found')
