 ...
 *
 * MainActivity remains the existing TruthRaw processor. FotoGraaf is the
 * acquisition/metrology entrance. Keeping both behind one launcher prevents a
 * temporary camera harness from becoming a replacement application lineage.
 */
class TruthRawSuiteLauncherActivity : Activity() {
    ...
    private fun buildUi(): LinearLayout {
        ...
        root.addView(TextView(this).apply {
            text = "De camera legt sealed Camera2 RAW + capture-observatie vast. De bestaande TruthRaw processor blijft de wetenschappelijke reconstructie- en exportlaag."
            textSize = 14f
            setTextColor(Color.rgb(170, 177, 188))
            setPadding(0, 0, 0, dp(22))
        })

        root.addView(actionButton("FotoGraaf camera openen") {
        ...
        root.addView(space())
        root.addView(TextView(this).apply {
            text = "Authority boundary: stock HONOR APK = technische route-hint. Alleen runtime Camera2-resultaat + sealed RAW kunnen capture-evidence leveren."
            textSize = 12f
