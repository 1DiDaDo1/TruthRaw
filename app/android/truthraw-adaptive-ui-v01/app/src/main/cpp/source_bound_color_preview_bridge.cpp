...
constexpr jint kMagic = 0x54524331; // TRC1
constexpr std::size_t kHeaderInts = 23;
...
Java_com_truthraw_adaptiveui_NativeTilePreviewBridge_buildSourceBoundColorPreview(
    ...
    if (!opened) return status_packet(env, dng_status(opened));
    ...
    options.memoryBudgetBytes = static_cast<std::size_t>(maxLogicalResidentBytes);

    StreamingResult streaming;
    const auto processed = processor.process(*source, sink, options, streaming);
    ...
    if (pixels.size() != expectedPixels || sink.writtenPixelCount() != expectedPixels) {
